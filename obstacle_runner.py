...
class Agent:
    ...
    def perceive(self, goal, obstacles):
        vel = np.array(
            [self.vx / 10.0, self.vy / 15.0, self.vz / 10.0], dtype=np.float32
        )
        dx = goal["x"] - self.x
        dz = goal["z"] - self.z
...
if __name__ == "__main__":
    ...
    if headless:
        ...
        # UI
        title_text = Text(
            "Level 1: First Steps", position=(-0.85, 0.45), scale=1.5, color=color.white
        )
        stats_text = Text(
            "Episode: 0 | Won: 0 | Eps: 100%",
            position=(-0.85, 0.38),
            scale=1,
            color=color.white,
        )
        help_text = Text(
            "[R] Reset  [N] Next Level  [H] Headless  [,/.] Epsilon",
            position=(-0.85, -0.47),
            scale=0.8,
            color=color.white,
        )
        loss_text = Text(
            "Loss: 0.0000", position=(-0.85, 0.32), scale=1, color=color.white
        )
        ...
        def update():
            ...
            while accumulator_state[0] >= physics_dt:
                ...
                agent.apply_action(action)
                reward, done = agent.physics_step(
                    physics_dt, lvl["platforms"], goal_pos
                )
                ...
                if last_state is not None:
                    ...
                    if len(replay) >= BATCH_SIZE:
                        ...
                        with torch.no_grad():
                            q_target = (
                                r_t + GAMMA * (1 - d_t) * target_net(ns_t).max(1)[0]
                            )
                        loss = loss_fn(q_sa, q_target)
                        optimizer.zero_grad()
                        loss.backward()
                ...
                if steps % 3 == 0:
                    tp = Entity(
                        model="sphere",
                        position=(agent.x, agent.y - 0.3, agent.z),
                        scale=0.1,
                ...
                epsilon = max(EPSILON_MIN, epsilon * EPSILON_DECAY)
                stats_text.text = (
                    f"Episode: {episode} | Won: {wins} | Eps: {epsilon:.0%}"
                )
